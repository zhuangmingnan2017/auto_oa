package com.yinian.autooa.dao.autocode;

import com.yinian.autooa.model.Schedule;
import com.yinian.autooa.model.ScheduleExample;
import java.util.List;
import org.apache.ibatis.annotations.Param;

public interface ScheduleMapper {
    /**
     *  根据指定的条件获取数据库记录数,schedule
     *
     * @param example
     */
    int countByExample(ScheduleExample example);

    /**
     *  根据指定的条件删除数据库符合条件的记录,schedule
     *
     * @param example
     */
    int deleteByExample(ScheduleExample example);

    /**
     *  根据主键删除数据库的记录,schedule
     *
     * @param id
     */
    int deleteByPrimaryKey(Integer id);

    /**
     *  新写入数据库记录,schedule
     *
     * @param record
     */
    int insert(Schedule record);

    /**
     *  动态字段,写入数据库记录,schedule
     *
     * @param record
     */
    int insertSelective(Schedule record);

    /**
     * ,schedule
     *
     * @param example
     */
    List<Schedule> selectByExampleWithBLOBs(ScheduleExample example);

    /**
     *  根据指定的条件查询符合条件的数据库记录,schedule
     *
     * @param example
     */
    List<Schedule> selectByExample(ScheduleExample example);

    /**
     *  根据指定主键获取一条数据库记录,schedule
     *
     * @param id
     */
    Schedule selectByPrimaryKey(Integer id);

    /**
     *  动态根据指定的条件来更新符合条件的数据库记录,schedule
     *
     * @param record
     * @param example
     */
    int updateByExampleSelective(@Param("record") Schedule record, @Param("example") ScheduleExample example);

    /**
     * ,schedule
     *
     * @param record
     * @param example
     */
    int updateByExampleWithBLOBs(@Param("record") Schedule record, @Param("example") ScheduleExample example);

    /**
     *  根据指定的条件来更新符合条件的数据库记录,schedule
     *
     * @param record
     * @param example
     */
    int updateByExample(@Param("record") Schedule record, @Param("example") ScheduleExample example);

    /**
     *  动态字段,根据主键来更新符合条件的数据库记录,schedule
     *
     * @param record
     */
    int updateByPrimaryKeySelective(Schedule record);

    /**
     * ,schedule
     *
     * @param record
     */
    int updateByPrimaryKeyWithBLOBs(Schedule record);

    /**
     *  根据主键来更新符合条件的数据库记录,schedule
     *
     * @param record
     */
    int updateByPrimaryKey(Schedule record);

    int insertBatchSelective(List<Schedule> records);
}